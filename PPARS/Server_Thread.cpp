...
mpz_class Server::SDRD(const Data& x1, const Data& x2, std::vector<QToken>& qt, std::map<int, StrSet>& encodeTable,
    ...
    std::vector<std::future<std::pair<mpz_class, mpz_class>>> futureAlphaBeta; // 用于存储并行任务的结果

    for (int i = 0; i < dimens; ++i) {
        futureAlphaBeta.push_back(std::async(std::launch::async, [this, &x1, &x2, &qt, &encodeTable, &resCache, i]() {
            ...
            return std::make_pair(ealpha, ebeta);
        }));
    }
    ...
    // 获取并处理每个维度的结果
    for (auto& future : futureAlphaBeta) {
        auto [ealpha, ebeta] = future.get();
