    ...
    for (int datasetSize : datasetSizes) {
        ...
        for (int r = 0; r < repeat; ++r) {
            ...
            for (int l = 0; l < loop; ++l) {
                ...
                std::vector<std::future<QToken>> futures;
                for (int i = 0; i < dimension; ++i) {
                    if (futures.size() >= fixedThreads) {
                        futures[i % fixedThreads].get();  // 等待线程完成，回收资源
                    }
                    futures.push_back(std::async(std::launch::async, [&shs, &sp, &qpoint, bitlen, domain, maxElementNum, hashfuncNum, i] {
                ...
                std::vector<std::future<mpz_class>> searchFutures;
                for (size_t i = 0; i < dataset.size(); ++i) {
                    if (searchFutures.size() >= fixedThreads) {
                        searchFutures[i % fixedThreads].get();  // 等待线程完成，回收资源
                    }
                    searchFutures.push_back(std::async(std::launch::async, [&shs, &server, &dataset, &QT, &encodeTable, &resCache, &pk, &sp, i] {
                        Data& xi = dataset[i];
...
int main() {
    ...
    std::vector<int> datasetSizes = { 1000, 1200, 1400, 1600, 1800, 2000 };  // 数据集大小
    int repeat = 2;  // 每组实验重复次数
